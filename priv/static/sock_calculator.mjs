...
// build/dev/javascript/sock_calculator/sock_calculator.ffi.mjs
function get_stitch_count() {
  const stitch_count_input = document.getElementById("stitch-count");
  const stitch_count = stitch_count_input?.value;
  return stitch_count ? new Ok(stitch_count) : new Error(void 0);
}
...
var GotSubmittedStitchCount = class extends CustomType {
  constructor(value3) {
    super();
    this.value = value3;
  }
...
function init2(_) {
  return [new Model(60, "60"), none()];
}
function get_stitch_count2() {
  return from2(
    (dispatch) => {
      let _pipe = get_stitch_count();
      let _pipe$1 = new GotSubmittedStitchCount(_pipe);
...
function update2(model, msg) {
  if (msg instanceof UserPressedKey && msg.value === "Enter") {
    ...
    return [model, get_stitch_count2()];
  } else if (msg instanceof UserResetStitchCount) {
    return [new Model(60, "60"), none()];
  } else if (msg instanceof GotSubmittedStitchCount && msg.value.isOk()) {
    let value3 = msg.value[0];
    let stitch_count = (() => {
      let _pipe = parse(value3);
      return unwrap(_pipe, 60);
    })();
    return [new Model(stitch_count, value3), none()];
  } else {
    return [model, none()];
  }
}
    ...
    stack2(
      ...
      toList([
        ...
        aside2(
          ...
          button3(
            toList([on_click(new UserSubmittedStitchCount())]),
